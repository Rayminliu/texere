...
从一份中文正式文档渲染管线抽离，2026-09-13 独立成包。运行期依赖只有 python-docx + lxml（外加外部 pandoc）。
...
> 272 页 / 252 表 / 112 图 → **69.7 / 66.9 秒**（连跑两次），产物 docx 2.9 MB，
> 两次结果逐项一致（页数、字数、表格、图片及抽查页像素全部相同），Word 进程无残留。
...
- 第一个 `#` **之前**别放内容（比如标题性的短语）：它既不会被当成封面，也会排在目录之后。
...
- **表单/附件类文档**（没有 `#` 标题）也能处理：不插目录、不分节，只做封面与表格/题注排版。
  这类表格的首行通常是字段名而非列标题，配 `"style": {"header_rows": 0}` 以免被灰底加粗。

> **契约：只改版式，不改内容。** 图表编号由源文件手写，`post.py` 不触碰任何正文与题注的文字。
> （0.2.0 期间曾有过「自动编号 + `@tab:` 交叉引用」功能，因会改写题注文字、
> 给无编号题注补号、且不同步手写引用，已**整体移除**。）
...
- 图表编号为**手写**：插删图表后需要人工对号（本工具不代改编号，见「用法」里的契约）；
