...
1. 它会**改写题注段落的文字**——题注内部若有局部加粗/斜体，会被合并掉；
2. 它会给**原本没写编号的题注补上编号**（例如 `附件 8-1 …` 会变成 `图 8-1 附件 8-1 …`，
   因为后者的前缀不是「图/表」而不被识别为编号）；
3. 它**不会同步正文里的引用文字**。正文若靠手写「（表2-2）」引用，重排编号后这些引用就错位了
   ——只有用 `@tab:` / `@fig:` 标签的引用才会跟着更新。

因此：**编号本来就已经正确的文档，建议不开 `auto_number`**（实测页数一致、原文一字不改）。
