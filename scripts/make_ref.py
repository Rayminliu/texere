...
_ap.add_argument(
    "--src",
    default=os.path.join(KIT, "assets", "ref_default.docx"),
    help="基准模板（默认 assets/ref_default.docx，缺失时由 pandoc 生成）",
)
_ap.add_argument(
    "--dst",
    default=os.path.join(KIT, "assets", "ref.docx"),
    help="输出模板（默认覆盖 assets/ref.docx）",
)
_ap.add_argument("--body-font", default="宋体", help="正文中文字体（默认 宋体）")
_ap.add_argument("--latin-font", default="Times New Roman", help="西文字体（默认 Times New Roman）")
_ap.add_argument("--heading-font", default="黑体", help="标题中文字体（默认 黑体）")
_ap.add_argument("--body-size", type=float, default=12.0, help="正文字号 pt（默认 12，即小四）")
_a = _ap.parse_args()
...
head_cfg = {
    "Heading 1": dict(
        size=16, align=WD_ALIGN_PARAGRAPH.CENTER, before=18, after=14, page_break=True
    ),
...
for name, cfg in head_cfg.items():
    try:
        ...
        continue
    set_font(st, east=HEI, latin=LATIN, size=cfg["size"], bold=True, color=(0x00, 0x00, 0x00))
    para(
        st,
        align=cfg["align"],
        before=cfg["before"],
        ...
        line=1.15,
        keep_next=True,
        page_break_before=cfg.get("page_break"),
    )
...
for name, size in (("Title", 26), ("Subtitle", 16)):
    try:
        ...
        continue
    set_font(st, east=HEI, size=size, bold=(name == "Title"), color=(0x00, 0x00, 0x00))
...
for name in ("TOC Heading", "TOC 1", "TOC 2", "TOC 3"):
    try:
        ...
        continue
    set_font(st, east=SONG, size=12, bold=(name == "TOC Heading"), color=(0x00, 0x00, 0x00))
